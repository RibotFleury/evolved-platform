...
export default function Footer({ locale = "fr" }: { locale?: string }) {
  ...
  const navLinks =
    locale === "fr"
      ? [
          { href: `/${locale}/about`, label: "Qui sommes-nous" },
    ...
    <footer className="border-t border-[var(--border)] bg-white">
        ...
        <div className="grid gap-10 md:grid-cols-3">
          ...
          <div className="space-y-4">
            ...
            <p className="max-w-xs text-sm leading-6 text-gray-600">
              {locale === "fr"
                ? "Nous catalysons la transformation numérique en Afrique en misant sur les talents locaux."
                : "We accelerate digital transformation in Africa by empowering local talent."}
            </p>
            ...
            <ul className="space-y-2">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-gray-600 transition hover:text-[var(--primary)]"
                  >
                    {link.label}
                  </Link>
                </li>
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              ...
              <Link
                href={`/${locale}/contact`}
                className="inline-flex items-center gap-2 rounded-xl bg-[var(--primary)] px-5 py-2.5 text-sm font-medium text-white transition hover:bg-[var(--primary-hover)]"
              >
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
              ...
              <div className="flex items-center gap-3">
                {socialLinks.map((social) => (
                  <a
                    ...
                    className="flex h-10 w-10 items-center justify-center rounded-full border border-[var(--primary)] text-[var(--primary)] transition duration-300 hover:-translate-y-1 hover:scale-110 hover:bg-[var(--primary)] hover:text-white hover:shadow-lg"
                  >
                    {social.icon}
                  </a>
                ))}
      ...
      <div className="border-t border-[var(--border)]">
        <div className="mx-auto flex max-w-6xl flex-col gap-2 px-4 py-4 text-xs text-gray-500 md:flex-row md:items-center md:justify-between">
          <p>
            © {currentYear}{" "}
            <span className="font-semibold text-black">EvolveD</span>.{" "}
